define([
	...
	describe('Directive: "action-links" ', function() {
		...
		beforeEach(function() {
			mocks.module('TT-UI.Common.Directives.ActionLinks');
		...
		it('Should test if element contains proper structure', function() {
			...
			expect(html.children().eq(0).text()).toMatch(states[0].self.label);
			expect(html.children().eq(1).text()).toMatch(states[1].self.label);
		});
