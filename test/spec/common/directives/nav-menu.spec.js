define([
	...
	describe('Directive: "nav-menu" ', function() {
		...
		beforeEach(function() {
			mocks.module('TT-UI.Common.Directives.NavMenu');

			mocks.module(function(_$stateProvider_) {
				$stateProvider = _$stateProvider_;
			});

			mocks.inject(function(_$compile_, _$rootScope_, _$state_, _$location_) {
				$compile   = _$compile_;
		...
		it('Should find proper root based on state name', function() {
			...
			expect(html.find('a').text()).toMatch($state.$current.children[0].self.label);
		});
	});
});