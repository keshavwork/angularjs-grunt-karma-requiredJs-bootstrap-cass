define([
	...
	describe('Directive: "tabs" ', function() {
		...
		beforeEach(function() {
			mocks.module('TT-UI.Common.Directives.Tabs');

			mocks.module(function(_$stateProvider_) {
				$stateProvider = _$stateProvider_;
			});

			mocks.inject(function(_$compile_, _$rootScope_, _$state_, _$location_, _$timeout_) {
				$compile   = _$compile_;
		...
		it('Should check if directive have procesed element', function() {
			...
			expect(html.prop('tagName').toLowerCase()).not.toMatch('tabset');
		});
		...
		it('Should test if element contains proper structure', function() {
			...
			expect(tabs.eq(0).children().text()).toMatch('Foo Tab');
			expect(tabs.eq(1).children().text()).toMatch('Bar Tab');
		});
