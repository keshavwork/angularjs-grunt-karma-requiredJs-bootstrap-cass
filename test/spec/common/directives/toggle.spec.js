define([
	...
	describe('Directive: "toggle" ', function() {
		...
		beforeEach(function() {
			mocks.module('TT-UI.Common.Directives.Toggle');
		...
		it('Should check if directive have procesed element', function() {
			...
			expect(html.prop('tagName').toLowerCase()).not.toMatch('toggle');
		});
		...
		it('Should test if element contains proper structure', function() {
			...
			expect(toggle.attr('for')).not.toMatch(null);
			expect(toggle.attr('for')).toMatch(input.attr('id'));
		});
